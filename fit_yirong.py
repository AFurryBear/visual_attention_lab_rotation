...
from fooof.core.funcs import gaussian_function, infer_ap_func
...
def get_ap_func_full(xs, *params):
    ...
    *params : float
        ...
        y = 10^offset1 * (1/(fk1^exp1 + x^exp1)+offset2/(fk2^exp2 + x^exp2))
    ...
    offset1, offset2, fk1, fk2, exp1, exp2 = params

    ys = ys + offset1 + np.log10(offset2 * (xs ** exp1 + fk1 ** exp1) + (xs ** exp2 + fk2 ** exp2)) - np.log10(
        xs ** exp2 + fk2 ** exp2) - np.log10(xs ** exp1 + fk1 ** exp1)

    return ys


def get_ap_func_simple(xs, *params):
    """Exponential fitting function, for fitting aperiodic component with a 'knee'.
    ...
    *params : float
    ...
    offset1, fk, exp1 = params

    ys = ys + offset1 - np.log10(xs ** exp1 + fk ** exp1)

    return ys


def get_ap_func_twoexp(xs, *params):
    """Exponential fitting function, for fitting aperiodic component with a 'knee'.
    ...
    *params : float
    ...
    offset1, fk, exp1, exp2 = params

    ys = ys + offset1 - np.log10(xs ** (exp2 + exp1) + fk ** (2 * exp1 + exp2) * xs ** (exp1))
...
class FOOOF_new():
    ...
    def __init__(self, peak_width_limits=(0.5, 12.0), max_n_peaks=np.inf, min_peak_height=0.0,
                 peak_threshold=2.0, aperiodic_mode='knee', verbose=True):
        ...
        self._ap_percentile_thresh = .025
        # Guess parameters for aperiodic fitting, [offset1, offset2, knee, exponent1, exponent2]
        ...
        self._ap_bounds_simple = ((0, 0, 0), (np.inf, np.inf, np.inf))
        self._ap_bounds_full = ((0, 0, 0, 0, 0, 0), (np.inf, np.inf, np.inf, np.inf, np.inf, np.inf))
    ...
    def _reset_internal_settings(self):
        ...
        if self.peak_width_limits:
            ...
            ## todo
            self._ap_bounds_full = self._ap_bounds_full
            self._ap_bounds_simple = self._ap_bounds_simple
        ...
        # Otherwise, assume settings are unknown (have been cleared) and set to None
            ...
            self._ap_bounds_full = None
            self._ap_bounds_simple = None
    ...
    def fit(self, freqs=None, power_spectrum=None, freq_range=None):
        ...
        try:
            ...
            self.aperiodic_params_ = self._robust_ap_fit(self.freqs[7:], self.power_spectrum[7:])
            self.aperiodic_params_ = self._final_ap_fit(self.freqs, self.power_spectrum)
            self._ap_fit = get_ap_func_full(self.freqs, *self.aperiodic_params_)
            ...
            self.aperiodic_params_ = self._final_ap_fit(self.freqs, self._spectrum_peak_rm, onefit=True)
            self._ap_fit = get_ap_func_full(self.freqs, *self.aperiodic_params_)
            self._spectrum_flat = self.power_spectrum - self._ap_fit
    ...
    def _final_ap_fit(self, freqs, power_spectrum, onefit=False):
        ...
        if len(self.aperiodic_params_) == 3:
            if self.aperiodic_params_[1] > 2:
                off_1 = np.min(power_spectrum[np.where(self.freqs < self.aperiodic_params_[1])[0]])
            else:
                off_1 = self.aperiodic_params_[0]
            aperiodic_params_ = (off_1, self.aperiodic_params_[1], self.aperiodic_params_[2])
            initial_fit = get_ap_func_simple(freqs, *aperiodic_params_)
            flatspec = power_spectrum - initial_fit
            flatspec[flatspec < 0] = 0
            fk_2 = 0
            off_2 = flatspec[0] / self.power_spectrum[0]
            exp_2 = np.abs(self.power_spectrum[3] - self.power_spectrum[0]) / (
                        np.log10(self.freqs[3]) - np.log10(self.freqs[0]))
            popt = (off_1, off_2, self.aperiodic_params_[1], fk_2, self.aperiodic_params_[2], exp_2)

        else:
            popt = self.aperiodic_params_
        print('===full guess===')
        print(['%.2f' % p for p in popt])
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                # todo
                aperiodic_params, _ = curve_fit(get_ap_func_full,
                                                freqs, power_spectrum, p0=popt,
                                                maxfev=self._maxfev, bounds=self._ap_bounds_full)
                print('===full fitting===')
                print(['%.2f' % p for p in aperiodic_params])
        ...
        if onefit:
            ...
            initial_fit = get_ap_func_full(freqs, *aperiodic_params)
            ...
            try:
                with warnings.catch_warnings():
                    ...
                    # todo
                    aperiodic_params, _ = curve_fit(get_ap_func_full,
                                                    freqs_ignore, spectrum_ignore, p0=aperiodic_params,
                                                    maxfev=self._maxfev, bounds=self._ap_bounds_full)
                    print('===final fitting===')
                    print(['%.2f' % p for p in aperiodic_params])
    ...
    def _simple_ap_fit(self, freqs, power_spectrum):
        ...
        off1_guess = [power_spectrum[0] if not self._ap_guess[0] else self._ap_guess[0]]
        knee_guess = [self._ap_guess[1]]
        exp1_guess = [np.abs(self.power_spectrum[-1] - self.power_spectrum[0] /
        ...
        # guess = np.array([off1_guess + off2_guess + kne_guess + exp1_guess + exp2_guess])
        guess = (off1_guess[0], knee_guess[0], exp1_guess[0])
        print('===initial guess===')
        print(['%.2f' % p for p in guess])
        #         print(guess.astype(.2f))
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                ## todo
                aperiodic_params, _ = curve_fit(get_ap_func_simple,
                                                freqs, power_spectrum, p0=guess,
                                                maxfev=self._maxfev, bounds=self._ap_bounds_simple)
    ...
    def _robust_ap_fit(self, freqs, power_spectrum):
        ...
        initial_fit = get_ap_func_simple(freqs, *popt)
        ...
        try:
            with warnings.catch_warnings():
                ...
                aperiodic_params, _ = curve_fit(get_ap_func_simple,
                                                ...
                                                maxfev=self._maxfev, bounds=self._ap_bounds_simple)
                #                 print(self._ap_bounds_full[1][2])
                #                 print(aperiodic_params[1]*4)
                self._ap_bounds_full = (self._ap_bounds_full[0],
                                        (self.power_spectrum[0] * 2,
                                         self.power_spectrum[0] * 2,
                                         aperiodic_params[1] + 10,
                                         5,
                                         aperiodic_params[2] * 5,
                                         aperiodic_params[2] * 5))
                #                 print(self._ap_bounds_full)
    ...
    def _fit_peaks(self, flat_iter):
        ...
        guess = np.empty([0, 3])
        self.min_peak_height = max(flat_iter) / 5
        ...
        while len(guess) < self.max_n_peaks:
            ...
            # self.min_peak_height = np.percentile(flat_iter, 25)
